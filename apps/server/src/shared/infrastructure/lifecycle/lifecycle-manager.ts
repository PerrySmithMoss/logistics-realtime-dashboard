...
import { AppState, ILifecycleManager } from "@shared/interfaces/lifecycle-manager.interface";
...
export class LifecycleManager implements ILifecycleManager {
  ...
  public setReady(): void {
    if (this._state !== AppState.STARTING) {
      throw new InternalServerError(`Cannot transition to READY from ${this._state}`, false);
    }
    this._state = AppState.READY;
  }

  public prepareForShutdown(): void {
    if (this._state === AppState.SHUTTING_DOWN || this._state === AppState.CLOSED) return;

    this.logger.info(`Transitioning state: ${this._state} -> ${AppState.SHUTTING_DOWN}`);

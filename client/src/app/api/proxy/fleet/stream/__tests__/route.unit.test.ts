...
describe("fleet stream proxy route", () => {
  ...
  it.each([401, 403])("passes through upstream auth failures for %s", async (status) => {
    global.fetch = vi.fn().mockResolvedValueOnce(new Response(null, { status })) as unknown as typeof fetch;
  ...
  it("maps non-auth upstream failures to 502", async () => {
    global.fetch = vi.fn().mockResolvedValueOnce(new Response(null, { status: 503 })) as unknown as typeof fetch;
  ...
  it("returns 204 when the upstream response has no stream body", async () => {
    global.fetch = vi.fn().mockResolvedValueOnce(new Response(null, { status: 204 })) as unknown as typeof fetch;

...
export const FleetMap = forwardRef<FleetMapHandle, FleetMapProps>(({ data }, ref) => {
  ...
  useEffect(() => {
    ...
    const source = mapInstance.getSource("vehicles") as maplibregl.GeoJSONSource;
    if (!source) return;
    ...
    if (
      ...
      const updatedFeature = data.features.find((f) => f.properties?.id === vehicleId);
      ...
      if (updatedFeature && updatedFeature.geometry.type === "Point") {
        ...
        currentPopup.setHTML(buildPopupHtml(updatedFeature.properties as FleetVehicle));
      }
    }
  }, [data]);
  ...
  return (
    <>
      <div ref={mapContainer} className="h-full w-full rounded-xl overflow-hidden shadow-inner" />
        ...
        {data.features.map((feature) => {
          ...
          return (
            <li key={properties.id}>
              <VehicleMarker vehicleId={properties.id} status={properties.status} />
            </li>
          );
        })}
      </ul>
